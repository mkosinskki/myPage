<template>
    <nav class="w-full sticky top-0 bg-offWhite shadow-md z-50">
        <div class="container max-w-6xl mx-auto px-5 py-5 flex items-center justify-between">
            ...
            <div class="z-50">
                <a href="#hero" class="text-2xl font-bold text-mediumGreen">
                    Marcin Kosiński
                </a>
            </div>

            <button id="menu-btn" type="button" class="z-50 block hamburger md:hidden focus:outline-none"
                :class="{ open: isMenuOpen }" @click="toggleMenu">
            ...
            <div class="hidden md:flex items-center gap-4 lg:gap-12">
                <component v-for="item in menuItems" :key="item.name" :is="item.path.startsWith('#') ? 'a' : RouterLink"
                    ...
                    <span class="relative z-10 bg-offWhite px-2.5 rounded-sm text-lg">{{ item.name }}</span>
                    <LeafPair side="right" />
                    <LeafPair side="left" />
                </component>
            </div>
        ...
        <Transition name="fade">
            <div v-if="isMenuOpen"
                ...
                <component v-for="item in menuItems" :key="item.name" :is="item.path.startsWith('#') ? 'a' : RouterLink"
                    ...
                    class="group relative text-graphite font-medium px-4 py-2 flex justify-center items-center text-xl w-full hover:bg-lightMint/20">
